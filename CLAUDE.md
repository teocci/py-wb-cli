...
| Content `/content/v2/cards/update` | Update semantics | Partial patch — send only the fields you change | **Destructive full-overwrite** — the card is *replaced*, so any omitted field is wiped. You must round-trip: `get/cards/list` the live card, swap only `description`, resend the whole object (required `nmID`+`vendorCode`+`sizes`, plus `brand`/`title`/`characteristics`/`dimensions`). Send `characteristics` as `{id, value}` only (drop the read `name`) and drop `dimensions.isValid`. `wb content apply/set-description` does this automatically (I-27). |
| Content `/content/v2/cards/update` | Success confirmation | HTTP 200 means the edit applied | **No** — a 200 only means the request was accepted. Per-card validation failures land in `/content/v2/cards/error/list`, **keyed by `vendorCode`** (not `nmID`). `wb content` polls it after each write and maps vendorCode→nmID (I-27). |
| Content `/content/v2/cards/update` | Description max length | Fixed 2000 | **Category-dependent** — standard 2000, minimum 1000, maximum 5000. The CLI only enforces the absolute 5000 ceiling (`too-long`); stricter per-category limits are enforced by WB and surface via `cards/error/list` (I-27). |
| Content `/content/v2/get/cards/list` | NM ID filtering | `filter` has an nmID field | **No nmID filter** — `filter` offers `textSearch` (article/WB article/SKU), `brands`, `objectIDs`, `tagIDs`, `imtID` only. `wb content get --nm` uses `textSearch` then matches exactly; `list --nms` fetches all and filters client-side. Set `withPhoto: -1` to avoid silently dropping photo-less cards (I-27). |
...
`wb content` reads and edits product **card descriptions** via the official Content API
(`content-api.wildberries.ru`, `content` token category). This is the *editable* card surface —
distinct from the read-only `wb portal products` scrape (basic metadata) and `wb product summary`
(ad/price analytics).

| Command | Endpoint | What it does |
|---------|----------|--------------|
| `wb content list` / `get` / `export` | `POST /content/v2/get/cards/list` | Read descriptions. `list` (table/JSON, `--nms`/`--text`/`--brand`), `get --nm` (one card's full text via `textSearch`), `export --out file.json` (all cards → editable `{nmID, vendorCode, title, description}` file). |
| `wb content apply --file f.json` | `POST /content/v2/cards/update` | Bulk-apply edits from a file. |
| `wb content set-description --nm N (--text\|--file)` | `POST /content/v2/cards/update` | Edit one card. |

**Writes are a read-modify-write round-trip** — see the `cards/update` quirk rows above: the live
card is re-fetched and only `description` is swapped, so structural fields survive the overwrite.
Every write supports `--dry-run` (diff only: `changed`/`unchanged`/`too-long`/`not-found`), prompts
unless `--yes`/`--json`, and confirms via `cards/error/list` after applying. The recommended flow
for a manager with many SKUs: `export` → edit the file → `apply --dry-run` → `apply`.
