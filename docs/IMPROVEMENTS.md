...
| 0.47.0 | I-26 | ✅ DONE | `wb economics product` — per-product unit economics joining warehouse stock (`get_warehouse_top`) with finance settlement rows (`detailed_sales_reports`). Settlement identity (live-verified): `bank_payment = Σ forPay − logistics − storage − deduction − penalty − acceptance`. Exact mode (default) shows only nmId-attributable costs (commission/acquiring/logistics); `--apportion` folds period storage+deductions pro-rata by revenue (`Σ net_payout == bank_payment`). Always emits an `EconomicsPeriod` reconciliation. `--scope in-stock\|sold\|all`, optional `--cogs-file` for net profit — [detail](phases/I-26-economics-unit-economics.md) |
| 0.48.0 | I-27 | ✅ DONE | `wb content` — read & edit product card descriptions via the official Content API (`content-api.wildberries.ru`). `list`/`get`/`export` read cards (`POST /content/v2/get/cards/list`, cursor-paginated); `apply`/`set-description` write via a **read-modify-write round-trip** against `POST /content/v2/cards/update` (a destructive full-overwrite — the live card is re-fetched, only `description` is swapped, the whole card is sent back so characteristics/sizes/dimensions are never wiped). `--dry-run` diffs without writing; post-update `POST /content/v2/cards/error/list` confirms success. Bulk export→edit→apply + single edit, so an agent picks per situation — [detail](phases/I-27-content-descriptions.md) |
