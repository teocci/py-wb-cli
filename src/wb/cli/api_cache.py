"""CLI diagnostic commands for the I-15 HTTP response cache.

``wb api-cache status`` and ``wb api-cache clear`` are the operator
equivalents of ``wb rate status`` for the request cache. They read
...
__all__ = ['api_cache_app']

api_cache_app = typer.Typer(
    help='HTTP response cache diagnostic',
    no_args_is_help=True,
)


@api_cache_app.command('status')
def api_cache_status(ctx: typer.Context) -> None:
...
def api_cache_clear(
        ...
        token_fp: str | None = typer.Option(
            ...
            help='Clear only entries for this token fingerprint prefix '
                 '(matches the values shown by `wb api-cache status`).',
        ),
...
def _build_fp_to_seller(store) -> dict[str, str | None]:
    ...
    seller_id from the JWT ``sid`` claim. Used by ``api-cache status``
    to annotate cache rows with a human-readable seller label.
    """
...
def _render_status_table(active_profile: str, sellers: list[dict]) -> None:
    """Print the human-readable view of ``api-cache status``."""
