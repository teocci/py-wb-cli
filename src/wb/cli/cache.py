"""CLI commands for local snapshot cache management."""
...
@cache_app.command('snapshot')
def cache_snapshot(
        ctx: typer.Context,
        campaign_id: int = typer.Option(
            ..., '--campaign', '-c', help='Campaign ID to snapshot',
        ),
...
@cache_app.command('snapshot-all')
def cache_snapshot_all(ctx: typer.Context) -> None:
    ...
    renderer.success(
        f'Captured {counts["campaigns"]} active campaign(s) to cache.'
    )
...
def cache_clear(
    ...
    if not (yes or renderer.is_json):
        confirmed = typer.confirm(
            f'Clear cached data for {scope} in profile {profile!r}?',
            default=False,
        )
        if not confirmed:
            raise typer.Abort()
