"""Service layer for WB Reports — warehouse remains with async polling."""
...
class ReportsService:
    ...
    and provides aggregation helpers.
    ...
    def get_warehouse_top(
        ...
        Returns:
            Top N ProductStockSummary objects sorted by total_quantity desc.
        ...
        if not task.is_done:
            raise ApiError(
                f'Warehouse report ended with status: {task.status}'
            )

        items = self.download_warehouse_report(task.task_id)
        return _aggregate_top(items, limit)
    ...
    def get_stock_runway(
        ...
        Returns:
            StockRunwayReport with per-product, per-warehouse runway data.
        ...
        stock_items = self._fetch_all_stock(poll_interval, poll_timeout)
        ...
        return StockRunwayReport(
            computed_at=computed_at,
            sales_period_days=sales_period_days,
            items=items,
        )

    def _fetch_all_stock(
            self,
            poll_interval: float,
            poll_timeout: float,
    ) -> list[WarehouseRemainItem]:
        """Create, poll, and download the full warehouse remains report."""
        task = self.create_warehouse_report(group_by_nm=True)
        logger.info('Created stock runway warehouse task: %s', task.task_id)
        task = self.poll_warehouse_report(
            task.task_id,
            interval=poll_interval,
            timeout=poll_timeout,
