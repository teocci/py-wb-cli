"""Tests for `wb cache status` and `wb cache clear` (HTTP response cache)."""
...
class TestCacheStatus:
    ...
    def test_empty_json_returns_empty_sellers(
        ...
        result = runner.invoke(app, ['--json', 'cache', 'status'])
        assert result.exit_code == 0
...
class TestCacheClear:
    def test_clear_all_with_yes_flag(
        ...
        result = runner.invoke(app, ['--json', 'cache', 'clear', '--all', '--yes'])
        assert result.exit_code == 0
        payload = json.loads(result.output)
    ...
    def test_clear_all_in_json_mode_skips_prompt(
        ...
        result = runner.invoke(app, ['--json', 'cache', 'clear', '--all'])
        assert result.exit_code == 0
        payload = json.loads(result.output)
